...
#***********************************************************************************************
# vars_for_template
def choice_vars_for_template(player, DAY_ABBREVIATIONS, NUM_ROUNDS, current_phase = 'V', choice='commute'):
    # === Basic Setup ===
    ...
    preview_data, total_avg, total_base, total_token_used = preview_overview_data(trips, weekly_choices=weekly_choices, day_abbreviations=DAY_ABBREVIATIONS)

    # === Mode Information for Current Trip ===
    modes = choice_set(poss_modes, trip, current_phase)
    ...
    # Add token-related values only if phase != 'I'
    if current_phase != 'I':
        context.update(dict(
...
#************************************
# before_next_page
def choice_before_next_page(player, NUM_ROUNDS, timeout_happened, current_phase='V', reduced=False, choice='commute'):
    # === Basic Setup ===
    trips = player.participant.vars['all_trips']
    trip_index = (player.round_number - 1) % len(trips)
    trip = trips[trip_index]
    ...
    if choice != 'no_commute':
        ...
        if current_phase != 'I':
            ...
            if token_needed > player.token:
                ...
                purchase_cost = token_to_buy * token_price + 10  # Add 10 DKK fixed fee
    ...
    if current_phase!='I':
        ...
        if player.round_number < NUM_ROUNDS:
            ...
            if (player.round_number + 1) % len(trips) == 1:
                if reduced == True: 
                    next_p.budget = player.participant.vars['initial_budget_reduced']
...
#***********************************************************************************************
# vars_for_template
def results_vars_for_template(player, NUM_ROUNDS, TRANSACTION_COSTS, PHASE_ORDER, DAY_ABBREVIATIONS, INITIAL_PRICE, reduced=False, current_phase = 'V'):
    ...
    week_end = week_start + len(trips)
    weekly_choices = trip_choices[week_start:week_end]
    preview_data, total_avg, total_base, _ = preview_overview_data(trips, weekly_choices=weekly_choices, day_abbreviations=DAY_ABBREVIATIONS)
    ...
    context = dict(
        ...
        preview_data=preview_data,
        next_week=next_week,
        budget_init = clean_zero(player.participant.vars['initial_budget'] if not reduced else player.participant.vars['initial_budget_reduced']),
