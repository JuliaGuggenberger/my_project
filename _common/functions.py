...
def preview_overview_data(trips, weekly_choices=None, day_abbreviations=None):
    ...
    for i, trip in enumerate(trips):
        ...
        avg_token = trip.get('tour_total_token_avg', 0)

        total_base += base_token
        total_avg += avg_token or 0
        ...
        preview_data.append({
            'day': trip['day'],
            'day_short':day_abbreviations[trip['day']],
            'mode': trip['mode'],
            'base_token': base_token,
            'avg_token': clean_zero(avg_token),
    ...
    return preview_data, total_avg, total_base, total_token_used

#***********************************************************************************************
# Choice set
def choice_set(poss_modes, trip, current_phase):
    modes = []
    for mode in poss_modes:
        ...
        if current_phase != 'I':
            modes.append({
                ...
                'dist': trip.get(f'tour_total_distance_km_{mode}'),
