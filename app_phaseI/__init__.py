...
class Player(BasePlayer):
    choice = models.StringField(label="Choose your transport mode")
...
class WeekPreview(Page):
    ...
    def vars_for_template(player):
        ...
        trips = player.participant.vars['all_trips']
        preview_data, _, _, _ = preview_overview_data(trips, weekly_choices=False, day_abbreviations=C.DAY_ABBREVIATIONS)
...
class Choice(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        return choice_vars_for_template(player, C.DAY_ABBREVIATIONS, C.NUM_ROUNDS, current_phase = 'I')
    
    @staticmethod
    def before_next_page(player, timeout_happened):
        return choice_before_next_page(player, C.NUM_ROUNDS, timeout_happened, current_phase = 'I')
...
class NoCommute(Page):
    ...
    @staticmethod
    def before_next_page(player, timeout_happened):
        return choice_before_next_page(player, C.NUM_ROUNDS, timeout_happened, current_phase = 'I', choice='no_commute')
...
class Results(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        return results_vars_for_template(player, C.NUM_ROUNDS, C.TRANSACTION_COSTS, C.PHASE_ORDER, C.DAY_ABBREVIATIONS, C.INITIAL_PRICE, current_phase='I')
